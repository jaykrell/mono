public class test2
{
    ...
    // duplicate of test_check_intrinsics but with local functions instead of MonoExt.
    //
    // The underlying feature is not yet present.

    public static int test_check_jit_optimization()
    {
        ...
        error |= check(error, "rol32r", -106 * k, 3, 4, (uint)0xC0000000 >> 2, RotateRight(3, 4));
        ...
        error |= check(error, "rol32r", -110 * k, 3, 4, (uint)0xC0000000 >> 2, RotateRight(3, 4 + not_constant_i));
        ...
        var data = test2data.data;
        if (k != test2data.k)
        {
            System.Console.WriteLine("test vs. testdata schema mismatch {0} {1}", k, test2data.k);
            return -1;
        }
    ...
    public static int test_check_intrinsics()
    {
        ...
        error |= check(error, "rol32r", -202 * k, 3, 4, (uint)0xC0000000 >> 2, MonoExt.RotateRight(3 + not_constant_ui, 4));
        ...
        error |= check(error, "rol32r", -206 * k, 3, 4, (uint)0xC0000000 >> 2, MonoExt.RotateRight(3, 4));
        ...
        error |= check(error, "rol32r", -210 * k, 3, 4, (uint)0xC0000000 >> 2, MonoExt.RotateRight(3, 4 + not_constant_i));
    ...
    public static int test_disassemble_all_shift_by_constant()
    ...
    public static int test_disassemble_all_constant()
    // This test always succeeds, but one should look at its disassembly
    // to determine that things are working.
    {
    ...
    {
        int err = test_disassemble_all_constant();
        err = (err != 0) ? err : test_disassemble_all_shift_by_constant();
        err = (err != 0) ? err : test_check_intrinsics();
        err = (err != 0) ? err : test_check_jit_optimization();
        if (err != 0)
