...
mono_string_get_pinned (MonoStringHandle str, MonoError *error)
{
	...
	memcpy (mono_string_handle_pin_chars (news, &new_gchandle),
		mono_string_handle_pin_chars (str, &str_gchandle),
...
mono_string_is_interned (MonoString *str_raw)
{
	...
	MONO_ENTER_GC_UNSAFE;
	str = mono_string_is_interned_lookup (str, FALSE, error);
	MONO_EXIT_GC_UNSAFE;
	mono_error_assert_ok (error);
...
mono_ldstr_metadata_sig (MonoDomain *domain, const char* sig, MonoError *error)
{
	...
	gunichar2 *p = (gunichar2*)mono_string_handle_pin_chars (o, &gchandle);
	for (gsize i = 0; i < len; ++i)
		p [i] = GUINT16_FROM_LE (p [i]);
	mono_gchandle_free_internal (gchandle);
#endif
	return mono_string_is_interned_lookup (o, TRUE, error);
}
