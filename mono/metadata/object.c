...
MonoStringHandle
mono_string_new_utf16_assign (MonoStringHandleInOut handle, MonoDomain *domain, const gunichar2 *text, gsize len, MonoError *error)
{
	MONO_HANDLE_ASSIGN_RAW (handle, mono_string_new_utf16_checked (domain, text, len, error));
	return handle;
}
...
mono_string_new_utf8_assign (MonoStringHandle o, MonoDomain *domain, const char *text, gsize length, MonoError *error)
{
	...
	if (eg_error) {
		...
		// FIXME? Like mono_ldstr_utf8:
		mono_error_set_argument (error, "string", eg_error->message);
		// FIXME? Like mono_string_new_checked:
