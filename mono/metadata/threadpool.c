...
worker_callback (void)
{
	...
	while (!mono_runtime_is_shutting_down ()) {
	    ...
	    MonoStringHandle thread_name = mono_string_new_handle (mono_get_root_domain (), "Threadpool worker", &error);
		mono_error_assert_ok (&error);
		mono_thread_set_name_handle_internal (thread, thread_name, FALSE, TRUE, &error);
		mono_error_assert_ok (&error);
