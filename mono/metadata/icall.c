...
{
	// func may be registered with multiple names.
	// For example:
	// name:__emul_fconv_to_u8 func:mono_fconv_u8 avoid_wrapper:false
	// name:__emul_fconv_to_u  func:mono_fconv_u8 avoid_wrapper:true
	//
	// The result is that __emul_fconv_to_u8 does not get a wrapper,
	// because last one wins (__emul_fconv_to_u) and specifies no wrapper.
	...
	MonoJitICallInfo *existing_info;

	if ((existing_info = (MonoJitICallInfo *)g_hash_table_lookup (jit_icall_hash_name, name))) {
		g_warning ("jit icall name already defined \"%s\" \"%s\" %p %p\n",
	...
	if ((existing_info = (MonoJitICallInfo *)g_hash_table_lookup (jit_icall_hash_addr, (gpointer)func))) {
		// mono_tls_get_lmf_addr, mono_get_lmf_addr
		// __emul_fconv_to_u, __emul_fconv_to_u8
		//
		//g_warning ("jit icall func already defined \"%s\" \"%s\" %p %p\n",
		//	name, existing_info->name, func, existing_info->func);
	}
