...
typedef enum MonoJitICallId {
	mono_jit_icall_id_invalid,
// While MONO_JIT_ICALL_ ## x may be more the style,
// whatever is used here needs to be a token
// paste from existing function names.
// _id may be too short subtle abstract.
// _index might be clearer and less abstract.
// id_ ## x ?
//
#define mono_jit_icall_name_to_id(x)	x ## _id
#define MONO_REGISTER_JIT_ICALL(x) mono_jit_icall_name_to_id (x),
...
typedef union MonoJitICallInfos { // FIXME "s" is too subtle?
	MonoJitICallInfo array [mono_jit_icall_count];
	struct {
		MonoJitICallInfo zero_is_reserved;
...
#define mono_jit_icall_info_from_name(x)  mono_jit_icall_info.name.x
#define mono_jit_icall_info_from_id(x) mono_jit_icall_info.array [x]	// assert?
#define mono_jit_icall_info_to_id(x) ((x) - mono_jit_icall_info.array)  // assert?
