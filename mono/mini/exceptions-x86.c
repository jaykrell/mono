...
get_throw_trampoline (const char *name, gboolean rethrow, gboolean llvm, gboolean corlib, gboolean llvm_abs, gboolean resume_unwind, MonoTrampInfo **info, gboolean aot, gboolean preserve_ips)
{
	...
	if (aot) {
		...
		code = mono_arch_emit_load_aotconst (start, code, &ji, MONO_PATCH_INFO_JIT_ICALL_ADDR, corlib ? &mono_jit_icall_info.mono_x86_throw_corlib_exception : &mono_jit_icall_info.mono_x86_throw_exception);
