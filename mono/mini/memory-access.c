	...
	if (mini_type_is_reference (m_class_get_byval_arg (klass))) { // Refs *MUST* be naturally aligned
		...
		if (!((dest->opcode == OP_ADD_IMM && dest->sreg1 == cfg->frame_reg) ||
			...
			if (!size_ins && (cfg->opt & MONO_OPT_INTRINS) && mini_emit_wb_aware_memcpy (cfg, klass, iargs, size, align)) {
				...
				if (size_ins)
					mono_emit_jit_icall (cfg, &mono_jit_icall_info.mono_gsharedvt_value_copy, iargs);
				else
					mono_emit_jit_icall (cfg, &mono_jit_icall_info.mono_value_copy_internal, iargs);
			} else {
				...
				mono_jit_icall_info.mono_gc_get_range_copy_func = mono_gc_get_range_copy_func ();
				mono_emit_jit_icall (cfg, &mono_jit_icall_info.mono_gc_get_range_copy_func, iargs);
			}
			return;
		}
	}
