...
// the info globally a small problem, and the icall_info does not need
// to be public, so instantiate it here instead.
...
// Wrapper to provide "mono_" prefix on associated data.
static float
mono_fmodf (float a, float b)
{
	return fmodf (a, b);
}
...
register_icalls (void)
{
	...
	register_opcode_emulation (OP_RREM, "__emul_rrem", "float float float", mono_fmodf, "mono_fmodf", FALSE);
#endif
	...
	// THere is no actual dynamic selection here. Perhaps the point is to avoid a loader interposition.
	mono_register_jit_icall_info_full (&mono_gc_wbarrier_range_copy_icall_info, mono_gc_get_range_copy_func (), "mono_gc_wbarrier_range_copy", "void ptr ptr int", NULL, TRUE, "mono_gc_wbarrier_range_copy");
