...
interp_exec_method_full (InterpFrame *frame, ThreadContext *context, FrameClauseArgs *clause_args, MonoError *error)
{
	...
	// FIXME move this to interp_exec_method_inner?
...
main_loop:
		...
		case MINT_CALL_VARARG: {
			...
			MonoMethodSignature * const csig = (MonoMethodSignature*) frame->imethod->data_items [ip [-3 + 2]];
		...
		case MINT_CALLI: {
			...
			MonoMethodSignature *csignature = (MonoMethodSignature*)frame->imethod->data_items [ip [-2 + 1]];
...
#define LDC(n) do { sp->data.i = (n); ++ip; ++sp; } while (0)
		...
		MINT_IN_CASE(MINT_VCALLVIRT_FAST) {
			...
			const gboolean is_void = ip [-3] == MINT_VCALL || ip [-3] == MINT_VCALLVIRT;
			if (!is_void) {
				/* need to handle typedbyref ... */
				*sp = *child_frame->retval;
				sp++;
