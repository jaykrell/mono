...
get_callee_llvmonly (EmitContext *ctx, LLVMTypeRef llvm_sig, MonoJumpInfoType type, gconstpointer data)
{
	...
	if (ctx->module->static_link && ctx->module->assembly->image != mono_get_corlib () && type == MONO_PATCH_INFO_JIT_ICALL) {
		g_assert (!"MONO_PATCH_INFO_JIT_ICALL2");
...
emit_cond_system_exception (EmitContext *ctx, MonoBasicBlock *bb, const char *exc_type, LLVMValueRef cmp)
{
	...
	if (!callee) {
		LLVMTypeRef sig;
		MonoJitICallInfo *icall_info = &mono_jit_icall_info.mono_llvm_throw_corlib_exception_abs_trampoline;
...
{
	MonoJitICallInfo * const icall_info = mono_get_aot_init_wrapper_icall_info (subtype);
	LLVMModuleRef lmodule = module->lmodule;
...
emit_gc_safepoint_poll (MonoLLVMModule *module)
{
	...
	ji = g_new0 (MonoJumpInfo, 1);
	ji->type = MONO_PATCH_INFO_JIT_ICALL_INFO;
	ji->data.icall_info = &mono_jit_icall_info.mono_threads_state_poll;
	ji = mono_aot_patch_info_dup (ji);
	got_offset = mono_aot_get_got_offset (ji);
	module->max_got_offset = MAX (module->max_got_offset, got_offset);
	indexes [0] = LLVMConstInt (LLVMInt32Type (), 0, FALSE);
...
{
	MonoJitICallInfo *icall_info = rethrow ? &mono_jit_icall_info.mono_llvm_rethrow_exception: &mono_jit_icall_info.mono_llvm_throw_exception;
...
emit_throw (EmitContext *ctx, MonoBasicBlock *bb, gboolean rethrow, LLVMValueRef exc)
{
	...
	MonoJitICallInfo *icall_info = rethrow ? &mono_jit_icall_info.mono_arch_rethrow_exception : &mono_jit_icall_info.mono_arch_throw_exception;
	const char *icall_name = icall_info->name;
...
{
	MonoJitICallInfo *icall_info = &mono_jit_icall_info.mono_llvm_resume_exception;
	const char *icall_name = icall_info->name;
...
{
	MonoJitICallInfo *icall_info = &mono_jit_icall_info.mono_llvm_clear_exception;
	const char *icall_name = icall_info->name;
...
{
	MonoJitICallInfo *icall_info = &mono_jit_icall_info.mono_llvm_load_exception;
	const char *icall_name = icall_info->name;
...
{
	MonoJitICallInfo *icall_info = &mono_jit_icall_info.mono_llvm_match_exception;
	const char *icall_name = icall_info->name;
...
get_mono_personality (EmitContext *ctx)
{
	...
	if (!use_debug_personality) {
		...
		if (ctx->cfg->compile_aot) {
			personality = get_callee (ctx, personality_type, MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_jit_icall_info.personality);
		} else {
			personality = LLVMAddFunction (ctx->lmodule, default_personality_name, personality_type);
			LLVMAddGlobalMapping (ctx->module->ee, personality, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_jit_icall_info.personality));
			mono_memory_barrier ();
		}
	}
...
#define ARM64_ATOMIC_FENCE_FIX
		...
		case OP_GC_SAFE_POINT: {
			...
			LLVMValueRef args [2];
			static LLVMTypeRef sig;
			MonoJitICallInfo *icall_info = &mono_jit_icall_info.mono_threads_state_poll;
			const char *icall_name = icall_info->name;
			g_assert (icall_name);
		...
		case OP_ENDFINALLY: {
			...
			if (ctx->llvm_only) {
				...
				if (ctx->cfg->compile_aot) {
					callee = get_callee (ctx, LLVMFunctionType (LLVMVoidType (), NULL, 0, FALSE), MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_jit_icall_info.mono_llvm_resume_unwind_trampoline);
				} else {
#if LLVM_API_VERSION > 100
