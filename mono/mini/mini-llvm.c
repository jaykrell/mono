...
emit_cond_system_exception (EmitContext *ctx, MonoBasicBlock *bb, const char *exc_type, LLVMValueRef cmp)
{
	...
	if (!callee) {
		LLVMTypeRef sig;
		MonoJitICallInfo *icall_info = &mono_jit_icall_info_mono_llvm_throw_corlib_exception_abs_trampoline;
...
mono_get_aot_init_wrapper_icall_info (MonoAotInitSubtype subtype)
{
	switch (subtype) {
		case AOT_INIT_METHOD:
			return &mono_jit_icall_info_init_method;
		case AOT_INIT_METHOD_GSHARED_MRGCTX:
			return &mono_jit_icall_info_init_method_gshared_mrgctx;
		case AOT_INIT_METHOD_GSHARED_THIS:
			return &mono_jit_icall_info_init_method_gshared_this;
		case AOT_INIT_METHOD_GSHARED_VTABLE:
			return &mono_jit_icall_info_init_method_gshared_vtable;
		default:
			g_assert_not_reached ();
	}
	return NULL;
...
{
	MonoJitICallInfo *icall_info = rethrow ? &mono_jit_icall_info_mono_llvm_rethrow_exception: &mono_jit_icall_info_mono_llvm_throw_exception;
...
emit_throw (EmitContext *ctx, MonoBasicBlock *bb, gboolean rethrow, LLVMValueRef exc)
{
	...
	MonoJitICallInfo *icall_info = rethrow ? &mono_jit_icall_info_mono_arch_rethrow_exception : &mono_jit_icall_info_mono_arch_throw_exception;
	const char *icall_name = icall_info->name;
	...
	if (!callee) {
		...
		if (ctx->cfg->compile_aot) {
			...
			target = resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO,
				rethrow ? &mono_jit_icall_info_mono_llvm_rethrow_exception_trampoline
					: &mono_jit_icall_info_mono_llvm_throw_exception_trampoline);
#else
...
{
	MonoJitICallInfo *icall_info = &mono_jit_icall_info_mono_llvm_resume_exception;
	const char *icall_name = icall_info->name;
...
{
	MonoJitICallInfo *icall_info = &mono_jit_icall_info_mono_llvm_clear_exception;
	const char *icall_name = icall_info->name;
...
{
	MonoJitICallInfo *icall_info = &mono_jit_icall_info_mono_llvm_load_exception;
	const char *icall_name = icall_info->name;
...
{
	MonoJitICallInfo *icall_info = &mono_jit_icall_info_mono_llvm_match_exception;
	const char *icall_name = icall_info->name;
...
get_mono_personality (EmitContext *ctx)
{
	...
	if (!use_debug_personality) {
		...
		mono_jit_icall_info_personality.name = default_personality_name;

		if (ctx->cfg->compile_aot) {
			personality = get_callee (ctx, personality_type, MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_jit_icall_info_personality);
		} else {
			personality = LLVMAddFunction (ctx->lmodule, default_personality_name, personality_type);
			LLVMAddGlobalMapping (ctx->module->ee, default_personality_name, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_jit_icall_info_personality));
			mono_memory_barrier ();
		}
	}
...
#define ARM64_ATOMIC_FENCE_FIX
		...
		case OP_GC_SAFE_POINT: {
			...
			LLVMValueRef args [2];
			static LLVMTypeRef sig;
			MonoJitICallInfo *icall_info = &mono_jit_icall_info_mono_threads_state_poll;
			const char *icall_name = icall_info->name;
			g_assert (icall_name);
		...
		case OP_ENDFINALLY: {
			...
			if (ctx->llvm_only) {
				...
				if (ctx->cfg->compile_aot) {
					callee = get_callee (ctx, LLVMFunctionType (LLVMVoidType (), NULL, 0, FALSE), MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_jit_icall_info_mono_llvm_resume_unwind_trampoline);
				} else {
#if LLVM_API_VERSION > 100
					gpointer target = (void*)mono_jit_icall_info_mono_llvm_resume_unwind_trampoline.func;
					g_assert (target);
...
#if LLVM_API_VERSION < 100
	MonoJitICallInfo *info = &mono_jit_icall_info_mono_llvm_resume_unwind_trampoline;
