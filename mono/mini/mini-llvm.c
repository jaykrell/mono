...
process_bb (EmitContext *ctx, MonoBasicBlock *bb)
{
	...
	for (ins = bb->code; ins; ins = ins->next) {
		...
		case OP_LOCALLOC: {
			...
			size = LLVMBuildAnd (builder, LLVMBuildAdd (builder,
convert (ctx, lhs, LLVMInt32Type ()), LLVMConstInt (LLVMInt32Type (), MONO_ARCH_FRAME_ALIGNMENT - 1, FALSE), ""),
LLVMConstInt (LLVMInt32Type (), ~ (MONO_ARCH_FRAME_ALIGNMENT - 1), FALSE), "");
