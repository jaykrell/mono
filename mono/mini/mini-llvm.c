...
get_callee_llvmonly (EmitContext *ctx, LLVMTypeRef llvm_sig, MonoJumpInfoType type, gconstpointer data)
{
	...
	if (ctx->module->assembly->image == mono_get_corlib () &&
			(type == MONO_PATCH_INFO_JIT_ICALL || type == MONO_PATCH_INFO_JIT_ICALL_INFO)) {
...
emit_cond_system_exception (EmitContext *ctx, MonoBasicBlock *bb, const char *exc_type, LLVMValueRef cmp)
{
	...
	if (!callee) {
		LLVMTypeRef sig;
		MonoJitICallInfo *icall_info = &mono_llvm_throw_corlib_exception_abs_trampoline_icall_info;
		...
		if (ctx->cfg->compile_aot) {
			callee = get_callee (ctx, sig, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info);
		} else {
			/*
			 ...
			 */
			gpointer target = resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info);
...
static LLVMValueRef
emit_init_icall_wrapper (MonoLLVMModule *module, MonoAotInitSubtype subtype, const char *name, MonoJitICallInfo *icall_info, int subtype)
{
	LLVMModuleRef lmodule = module->lmodule;
...
emit_gc_safepoint_poll (MonoLLVMModule *module)
{
	...
	LLVMDisposeBuilder (builder);
=======
	module->init_method = emit_init_icall_wrapper (module, "init_method", &mono_aot_init_llvm_method_icall_info, 0);
	module->init_method_gshared_mrgctx = emit_init_icall_wrapper (module, "init_method_gshared_mrgctx", &mono_aot_init_gshared_method_mrgctx_icall_info, 1);
	module->init_method_gshared_this = emit_init_icall_wrapper (module, "init_method_gshared_this", &mono_aot_init_gshared_method_this_icall_info, 2);
	module->init_method_gshared_vtable = emit_init_icall_wrapper (module, "init_method_gshared_vtable", &mono_aot_init_gshared_method_vtable_icall_info, 3);
...
process_call (EmitContext *ctx, MonoBasicBlock *bb, LLVMBuilderRef *builder_ref, MonoInst *ins)
{
	...
	if (ins->flags & MONO_INST_HAS_METHOD) {
		...
		if (info) {
			...
			*/
			if (cfg->compile_aot) {
				callee = get_callee (ctx, llvm_sig, MONO_PATCH_INFO_JIT_ICALL_INFO, info);
				if (!callee) {
					set_failure (ctx, "can't encode patch");
					return;
				}
...
{
	MonoJitICallInfo *icall_info = rethrow ? &mono_llvm_rethrow_exception_icall_info : &mono_llvm_throw_exception_icall_info;
	const char *icall_name = icall_info->name;
	...
	if (!callee) {
		...
		if (ctx->cfg->compile_aot) {
			...
			LLVMAddGlobalMapping (ctx->module->ee, callee, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info));
			mono_memory_barrier ();
...
emit_throw (EmitContext *ctx, MonoBasicBlock *bb, gboolean rethrow, LLVMValueRef exc)
{
	...
	MonoJitICallInfo *icall_info = rethrow ? &mono_arch_rethrow_exception_icall_info : &mono_arch_throw_exception_icall_info;
	const char *icall_name = icall_info->name;
	...
	if (!callee) {
		...
		if (ctx->cfg->compile_aot) {
			callee = get_callee (ctx, sig_to_llvm_sig (ctx, throw_sig), MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info);
		} else {
			gpointer target;
#ifdef TARGET_X86
			/* 
			 ...
			 * trampoline.
			 */
			target = resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO,
				rethrow ? &mono_llvm_rethrow_exception_trampoline_icall_info
					: &mono_llvm_throw_exception_trampoline_icall_info);
#else
			target = resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info);
#endif
...
{
	MonoJitICallInfo *icall_info = &mono_llvm_resume_exception_icall_info;
	const char *icall_name = icall_info->name;
	...
	if (!callee) {
		if (ctx->cfg->compile_aot) {
			callee = get_callee (ctx, fun_sig, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info);
		} else {
			callee = LLVMAddFunction (ctx->lmodule, icall_name, fun_sig);
			LLVMAddGlobalMapping (ctx->module->ee, callee, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info));
			mono_memory_barrier ();
...
{
	MonoJitICallInfo *icall_info = &mono_llvm_clear_exception_icall_info;
	const char *icall_name = icall_info->name;
	...
	if (!callee) {
		if (ctx->cfg->compile_aot) {
			callee = get_callee (ctx, call_sig, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info);
		} else {
...
{
	MonoJitICallInfo *icall_info = &mono_llvm_load_exception_icall_info;
	const char *icall_name = icall_info->name;
...
{
	MonoJitICallInfo *icall_info = &mono_llvm_match_exception_icall_info;
	const char *icall_name = icall_info->name;
	...
	if (!callee) {
		if (ctx->cfg->compile_aot) {
			...
			callee = get_callee (ctx, match_sig, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info);
		} else {
			callee = ctx->module->match_exc = LLVMAddFunction (ctx->lmodule, icall_name, match_sig);
			LLVMAddGlobalMapping (ctx->module->ee, ctx->module->match_exc, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO, icall_info));
...
// Slight hack to eliminate last string-based MONO_PATCH_INFO_JIT_ICALL.
static MonoJitICallInfo mono_default_personality_name_icall_info;
...
get_mono_personality (EmitContext *ctx)
{
	...
	if (!use_debug_personality) {
		...
		mono_default_personality_name_icall_info.name = default_personality_name;

		if (ctx->cfg->compile_aot) {
			personality = get_callee (ctx, personality_type, MONO_PATCH_INFO_JIT_ICALL_INFO, &default_personality_name_icall_info);
		} else {
			personality = LLVMAddFunction (ctx->lmodule, default_personality_name, personality_type);
			LLVMAddGlobalMapping (ctx->module->ee, personality, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL_INFO, &default_personality_name_icall_info));
			mono_memory_barrier ();
		}
	}
...
#define ARM64_ATOMIC_FENCE_FIX
		...
		case OP_GC_SAFE_POINT: {
			...
			LLVMValueRef args [2];
			static LLVMTypeRef sig;
			MonoJitICallInfo *icall_info = &mono_threads_state_poll_icall_info;
			const char *icall_name = icall_info->name;
			g_assert (icall_name);
		...
		case OP_ENDFINALLY: {
			...
			if (ctx->llvm_only) {
				...
				if (ctx->cfg->compile_aot) {
					callee = get_callee (ctx, LLVMFunctionType (LLVMVoidType (), NULL, 0, FALSE), MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_llvm_resume_unwind_trampoline_icall_info);
				} else {
#if LLVM_API_VERSION > 100
					gpointer target = (void*)mono_llvm_resume_unwind_trampoline_icall_info.func;
					g_assert (target);
...
#if LLVM_API_VERSION < 100
	MonoJitICallInfo *info = &mono_llvm_resume_unwind_trampoline_icall_info;
