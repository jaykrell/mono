...
get_callee_llvmonly (EmitContext *ctx, LLVMTypeRef llvm_sig, MonoJumpInfoType type, gconstpointer data)
{
	...
	if (ctx->module->static_link && ctx->module->assembly->image != mono_get_corlib () && type == MONO_PATCH_INFO_JIT_ICALL) {
		...
		if (info->func != info->wrapper) {
			...
			data = mono_icall_get_wrapper_method ((MonoJitICallId)GPOINTER_TO_UINT(data));
	...
	if (ctx->module->assembly->image == mono_get_corlib () &&
			(type == MONO_PATCH_INFO_JIT_ICALL || type == MONO_PATCH_INFO_JIT_ICALL)) {
...
emit_cond_system_exception (EmitContext *ctx, MonoBasicBlock *bb, const char *exc_type, LLVMValueRef cmp)
{
	...
	if (!callee) {
		LLVMTypeRef sig;
		MonoJitICallId icall_id = mono_jit_icall_name_to_id (mono_llvm_throw_corlib_exception_abs_trampoline);
		...
		if (ctx->cfg->compile_aot) {
			callee = get_callee (ctx, sig, MONO_PATCH_INFO_JIT_ICALL, icall_id);
		} else {
			/*
			 ...
			 */
			gpointer target = resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL, icall_id);
...
mono_get_aot_init_wrapper_icall_info (MonoAotInitSubtype subtype)
{
	switch (subtype) {
		case AOT_INIT_METHOD:
			return mono_jit_icall_name_to_id (init_method);
		case AOT_INIT_METHOD_GSHARED_MRGCTX:
			return mono_jit_icall_name_to_id (init_method_gshared_mrgctx);
		case AOT_INIT_METHOD_GSHARED_THIS:
			return mono_jit_icall_name_to_id (init_method_gshared_this);
		case AOT_INIT_METHOD_GSHARED_VTABLE:
			return mono_jit_icall_name_to_id (init_method_gshared_vtable);
		default:
			g_assert_not_reached ();
	}
	return mono_jit_icall_id_invalid;
}
...
{
	MonoJitICallId icall_id = rethrow ? mono_jit_icall_name_to_id (mono_llvm_rethrow_exception) : mono_jit_icall_name_to_id (mono_llvm_throw_exception);
	const char *icall_name = mono_jit_icall_info_from_id (icall_id).name;
	...
	if (!callee) {
		...
		if (ctx->cfg->compile_aot) {
			...
			LLVMAddGlobalMapping (ctx->module->ee, callee, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL, icall_id));
			mono_memory_barrier ();
...
emit_throw (EmitContext *ctx, MonoBasicBlock *bb, gboolean rethrow, LLVMValueRef exc)
{
	...
	MonoJitICallId icall_id = rethrow ? mono_jit_icall_name_to_id (mono_arch_rethrow_exception) : mono_jit_icall_name_to_id (mono_arch_throw_exception);
	const char *icall_name = mono_jit_icall_info_from_id (icall_id).name;
	...
	if (!callee) {
		...
		if (ctx->cfg->compile_aot) {
			callee = get_callee (ctx, sig_to_llvm_sig (ctx, throw_sig), MONO_PATCH_INFO_JIT_ICALL, icall_id);
		} else {
			gpointer target;
#ifdef TARGET_X86
			/* 
			...
			target = resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL,
				rethrow ? mono_jit_icall_name_to_id (mono_llvm_rethrow_exception_trampoline)
					: mono_jit_icall_name_to_id (mono_llvm_throw_exception_trampoline);
#else
			target = resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL, icall_id);
#endif
...
{
	MonoJitICallId icall_id = mono_jit_icall_name_to_id (mono_llvm_resume_exception);
	const char *icall_name = mono_jit_icall_info_from_id (icall_id).name;
	...
	if (!callee) {
		if (ctx->cfg->compile_aot) {
			callee = get_callee (ctx, fun_sig, MONO_PATCH_INFO_JIT_ICALL, icall_id);
		} else {
			callee = LLVMAddFunction (ctx->lmodule, icall_name, fun_sig);
			LLVMAddGlobalMapping (ctx->module->ee, callee, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL, icall_id));
			mono_memory_barrier ();
...
{
	MonoJitICallId icall_id = mono_jit_icall_name_to_id (mono_llvm_clear_exception);
	const char *icall_name = mono_jit_icall_info_from_id (icall_id).name;
	...
	if (!callee) {
		if (ctx->cfg->compile_aot) {
			callee = get_callee (ctx, call_sig, MONO_PATCH_INFO_JIT_ICALL, icall_id);
		} else {
...
{
	MonoJitICallId icall_id = mono_jit_icall_name_to_id (mono_llvm_load_exception);
	const char *icall_name = mono_jit_icall_info_from_id (icall_id).name;
...
{
	MonoJitICallId icall_id = mono_jit_icall_name_to_id (mono_llvm_match_exception);
	const char *icall_name = mono_jit_icall_info_from_id (icall_id).name;
	...
	if (!callee) {
		if (ctx->cfg->compile_aot) {
			...
			callee = get_callee (ctx, match_sig, MONO_PATCH_INFO_JIT_ICALL, icall_id);
		} else {
			callee = ctx->module->match_exc = LLVMAddFunction (ctx->lmodule, icall_name, match_sig);
			LLVMAddGlobalMapping (ctx->module->ee, ctx->module->match_exc, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL, icall_id));
...
get_mono_personality (EmitContext *ctx)
{
	...
	if (!use_debug_personality) {
		...
		mono_jit_icall_info.name.personality.name = default_personality_name;

		if (ctx->cfg->compile_aot) {
			personality = get_callee (ctx, personality_type, MONO_PATCH_INFO_JIT_ICALL, mono_jit_icall_name_to_id (personality);
		} else {
			personality = LLVMAddFunction (ctx->lmodule, default_personality_name, personality_type);
			LLVMAddGlobalMapping (ctx->module->ee, default_personality_name, resolve_patch (ctx->cfg, MONO_PATCH_INFO_JIT_ICALL, mono_jit_icall_name_to_id (personality));
			mono_memory_barrier ();
		}
	}
...
#define ARM64_ATOMIC_FENCE_FIX
		...
		case OP_GC_SAFE_POINT: {
			...
			LLVMValueRef args [2];
			static LLVMTypeRef sig;
			MonoJitICallId icall_id = mono_jit_icall_name_to_id (mono_threads_state_poll);
			const char *icall_name = mono_jit_icall_info_from_id (icall_id).name;
			g_assert (icall_name);
		...
		case OP_ENDFINALLY: {
			...
			if (ctx->llvm_only) {
				...
				if (ctx->cfg->compile_aot) {
					callee = get_callee (ctx, LLVMFunctionType (LLVMVoidType (), NULL, 0, FALSE), MONO_PATCH_INFO_JIT_ICALL, mono_jit_icall_name_to_id (mono_llvm_resume_unwind_trampoline);
				} else {
#if LLVM_API_VERSION > 100
					gpointer target = (void*)mono_jit_icall_info.name.mono_llvm_resume_unwind_trampoline.func;
					g_assert (target);
...
#if LLVM_API_VERSION < 100
	MonoJitICallInfo *info = mono_jit_icall_name_to_id (mono_llvm_resume_unwind_trampoline;
