			...
			for (ins = bb->code; ins; ins = ins->next) {
				...
				case OP_NEWARR:
					if (cfg->opt & MONO_OPT_SHARED) {
						...
						dest = mono_emit_jit_icall (cfg, &mono_jit_icall_info.ves_icall_array_new, iargs);
						dest->dreg = ins->dreg;
					} else {
						...
						if (managed_alloc)
							...
							dest = mono_emit_jit_icall (cfg, &mono_jit_icall_info.ves_icall_array_new_specific, iargs);
						dest->dreg = ins->dreg;
					}
					break;
				case OP_STRLEN:
		...
		while (restart) {
			...
			for (ins = bb->code; ins; ins = ins->next) {
				...
				case OP_STORER4_MEMBASE_REG: {
					...
					mono_emit_jit_icall (cfg, &mono_jit_icall_info.mono_fstore_r4, iargs);
					restart = TRUE;
					break;
				}
				case OP_LOADR4_MEMBASE: {
					...
					conv = mono_emit_jit_icall (cfg, &mono_jit_icall_info.mono_fload_r4, iargs);
					conv->dreg = ins->dreg;
					break;
				}					
				case OP_FCALL:
				...
				case OP_CKFINITE: {
					...
					call = mono_emit_jit_icall (cfg, &mono_jit_icall_info.mono_isfinite_double, iargs);
