...
mono_arch_output_basic_block (MonoCompile *cfg, MonoBasicBlock *bb)
{
	...
	//		g_print ("cil code\n");
		...
		case OP_GC_SAFE_POINT: {
			...
			buf [0] = code;
			ARM_B_COND (code, ARMCOND_EQ, 0);
			mono_add_patch_info (cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_jit_icall_info_mono_threads_state_poll);
			code = emit_call_seq (cfg, code);
...
mono_arch_emit_prolog (MonoCompile *cfg)
{
	...
	/*
	{
		...
		mono_add_patch_info (cfg, code - cfg->native_code, MONO_PATCH_INFO_JIT_ICALL_INFO, &mono_jit_icall_info_mono_arm_unaligned_stack);
		code = emit_call_seq (cfg, code);
		arm_patch (buf [0], code);
	}
	*/
...
mono_arch_emit_exceptions (MonoCompile *cfg)
{
	...
	for (patch_info = cfg->patch_info; patch_info; patch_info = patch_info->next) {
		...
		case MONO_PATCH_INFO_EXC: {
			...
			patch_info->data.icall_info = &mono_jit_icall_info_mono_arch_throw_corlib_exception;
