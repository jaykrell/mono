...
encode_patch (MonoAotCompile *acfg, MonoJumpInfo *patch_info, guint8 *buf, guint8 **endbuf)
{
	...
	case MONO_PATCH_INFO_JIT_ICALL_INFO:
		...
		name = patch_info->data.icall_info->name;
		goto handle_name;
	case MONO_PATCH_INFO_JIT_ICALL:
		g_assert (!"MONO_PATCH_INFO_JIT_ICALL");
	case MONO_PATCH_INFO_JIT_ICALL_ADDR:
...
static MonoJitICallInfo * const preinited_jit_icalls[] = {
	&mono_jit_icall_info_mini_llvm_init_method,
	&mono_jit_icall_info_mini_llvm_init_gshared_method_this,
	&mono_jit_icall_info_mini_llvm_init_gshared_method_mrgctx,
	&mono_jit_icall_info_mini_llvm_init_gshared_method_vtable,
	&mono_jit_icall_info_mini_llvmonly_throw_nullref_exception,
	&mono_jit_icall_info_mono_llvm_throw_corlib_exception,
	&mono_jit_icall_info_mono_threads_state_poll,
	&mono_jit_icall_info_mini_llvmonly_init_vtable_slot,
	&mono_jit_icall_info_mono_helper_ldstr_mscorlib,
	&mono_jit_icall_info_mono_fill_method_rgctx,
	&mono_jit_icall_info_mono_fill_class_rgctx
};
...
mono_compile_assembly (MonoAssembly *ass, guint32 opts, const char *aot_options, gpointer **global_aot_state)
{
	...
	if (mono_aot_mode_is_interp (&acfg->aot_opts) && mono_is_corlib_image (acfg->image->assembly->image)) {
		MonoMethod *wrapper = mini_get_interp_lmf_wrapper (&mono_jit_icall_info_mono_interp_to_native_trampoline);
		add_method (acfg, wrapper);

		wrapper = mini_get_interp_lmf_wrapper (&mono_jit_icall_info_mono_interp_entry_from_trampoline);
		add_method (acfg, wrapper);
