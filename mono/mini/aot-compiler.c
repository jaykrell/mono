...
add_wrappers (MonoAotCompile *acfg)
{
	...
	if (mono_is_corlib_image (acfg->image->assembly->image)) {
		...
		for (int i = 0; i < G_N_ELEMENTS (mono_jit_icall_info.array); ++i)
		...
		if (patch_info && (patch_info->ip.i == i) && (pindex < patches->len)) {
			...
			default: {
				...
				if ((patch_info->type == MONO_PATCH_INFO_METHOD) && (m_class_get_image (patch_info->data.method->klass) == acfg->image)) {
					...
					MonoJitICallInfo *info = &mono_jit_icall_info_from_id (patch_info->data.icall_id);
					const char *sym = info->c_symbol;
...
encode_patch (MonoAotCompile *acfg, MonoJumpInfo *patch_info, guint8 *buf, guint8 **endbuf)
{
	...
	case MONO_PATCH_INFO_SET_TLS_TRAMP:
		...
		break;
	case MONO_PATCH_INFO_JIT_ICALL:
		encode_value (patch_info->data.icall_id, p, &p);
		break;
	case MONO_PATCH_INFO_JIT_ICALL_ADDR:
...
get_plt_entry_debug_sym (MonoAotCompile *acfg, MonoJumpInfo *ji, GHashTable *cache)
{
	...
	case MONO_PATCH_INFO_METHOD:
		...
		break;
	case MONO_PATCH_INFO_JIT_ICALL:
		debug_sym = g_strdup_printf ("%s_jit_icall_%s", prefix, mono_jit_icall_info_from_id (ji->data.icall_id));
		break;
	case MONO_PATCH_INFO_RGCTX_FETCH:
...
mono_aot_get_direct_call_symbol (MonoJumpInfoType type, gconstpointer data)
{
	...
	if (llvm_acfg->aot_opts.direct_icalls) {
		if (type == MONO_PATCH_INFO_JIT_ICALL_ADDR) {
			...
			MonoJitICallInfo *info = &mono_jit_icall_info_from_id ((GPOINTER_TO_UINT)data);
			const char *name = info->c_symbol;
...
static MonoJitICallId const preinited_jit_icalls[] = {
	mono_jit_icall_name_to_id (mini_llvm_init_method),
	mono_jit_icall_name_to_id (mini_llvm_init_gshared_method_this),
	mono_jit_icall_name_to_id (mini_llvm_init_gshared_method_mrgctx),
	mono_jit_icall_name_to_id (mini_llvm_init_gshared_method_vtable),
	mono_jit_icall_name_to_id (mini_llvmonly_throw_nullref_exception),
	mono_jit_icall_name_to_id (mono_llvm_throw_corlib_exception),
	mono_jit_icall_name_to_id (mono_threads_state_poll),
	mono_jit_icall_name_to_id (mini_llvmonly_init_vtable_slot),
	mono_jit_icall_name_to_id (mono_helper_ldstr_mscorlib),
	mono_jit_icall_name_to_id (mono_fill_method_rgctx),
	mono_jit_icall_name_to_id (mono_fill_class_rgctx),
};
...
add_preinit_got_slots (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < sizeof (preinited_jit_icalls) / sizeof (char*); ++i) {
		...
		ji->data.icall_id = preinited_jit_icalls [i];
		add_preinit_slot (acfg, ji);
	}
...
mono_compile_assembly (MonoAssembly *ass, guint32 opts, const char *aot_options, gpointer **global_aot_state)
{
	...
	if (mono_aot_mode_is_interp (&acfg->aot_opts) && mono_is_corlib_image (acfg->image->assembly->image)) {
		MonoMethod *wrapper = mini_get_interp_lmf_wrapper (mono_jit_icall_name_to_id (mono_interp_to_native_trampoline));
		add_method (acfg, wrapper);

		wrapper = mini_get_interp_lmf_wrapper (mono_jit_icall_name_to_id (mono_interp_entry_from_trampoline));
		add_method (acfg, wrapper);
